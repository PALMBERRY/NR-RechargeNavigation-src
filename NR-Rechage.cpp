...
// *********************************************************************************************//
// ******	Name:		NR-RechargeNavigation Module *******************************************//
// ******	Author:		Njrobot Team				 *******************************************//
// ******	Vison:		V1.0						 *******************************************//
// ******	Copyright:	Njrobot						 *******************************************//
// *********************************************************************************************//
...
NJRobot::Logger* Recharge_logger = new NJRobot::Logger("RECHARGE");
...
double DistYtoDockStation;// 520 // 60cm'
double recheck_dist;
double leave_station_dist;
double the_last_dist;
double pre_minX;

// 25cm
//#define Ashort 482.963
//#define ConstSegShort 250
// 20cm
...
#define Conf_distmax 760					//小车调整确认距离
#define Conf_distmin 730
...
bool enter_flag = false, leave_flag = false, back_flag = false, last_flag = false, filter_flag = false;
int filter_num = 0;
double sum_x = 0, sum_y = 0, sum_theta = 0, sum_HX_theta = 0;
bool leave_flagTo = 0;
int weight;
int no_find_times;
int back_time_flag;
...
static int All_time = 0, Sucess_time = 0, Sucess_time_nd = 0;
...
int FitLineZone(int pt1, int pt2, Point_t *ScanPoints, int tinyDeg)
{
	...
	if (distAB < 500) //50cm
		SEG_DLineDistThreshold = 33; // mm'2e'eg.30mm' 33mm
...
void updateOdom(Mesg_RobotState state_odata)
{
	if(g_mtx_odom.try_lock())
	{
		...
		// 出站处理部分
		// 利用里程计，收到出站任务后，直直后退固定距离
		// 注意参数配置
		// TODO: 后退中的停障处理

		if(leave_flag)
		{
			deltaPose = absoluteDifference(curLeave_odom, preLeave_odom); //以old_odm为原点的motion
			curLeave_pose = absoluteSum(curLeave_pose,deltaPose);
			preLeave_odom = curLeave_odom;
			//	 printf("BACK_pose:--------------%lf %lf-----------------\n",curLeave_pose.x,curLeave_pose.y);
			cur_vel.set_vx(-0.1);
			cur_vel.set_vy(0);
			cur_vel.set_w(0);
			leave_dist = curLeave_pose.x*curLeave_pose.x + curLeave_pose.y*curLeave_pose.y;
			SubPubManager::Instance()->m_robotspeed.GetPublisher()->publish(cur_vel);

			if(leave_dist>leave_station_dist*leave_station_dist)
			{
				cur_vel.set_vx(0);
				cur_vel.set_vy(0);
				cur_vel.set_w(0);
				...
				LOG_INFO(Recharge_logger,"Leave Station Successfully! Leave_dist:"<<leave_dist);
...
void updateLaser(Mesg_Laser LRFdata2D)
{
	...
	if(Recharge_enable)
	{
		...
		if(g_mutex_laser.try_lock())
		{
			...
			double miX_nH = midXfound * cos(rotateXYrad) + midYtoCOG * sin(rotateXYrad) - 5.0;
			double miY_nH = -midXfound * sin(rotateXYrad) + midYtoCOG * cos(rotateXYrad);

			if (ret==1 && enter_flag){;
			// printf("\n\n theta_m %lf, theta %lf", theta2, theta);
			//printf("midXnH,YnH %lf,%lf, theta %lf, HX %lf\n", miX_nH, miY_nH, theta, HX_theta);
			...
			static std::vector<MedFilter<double> > filter(4,MedFilter<double>(5));

			if(!filter_flag)
			{
				filter_num ++;
				filter[0].input(miX_nH);
				filter[1].input(miY_nH);
				filter[2].input(theta);
				filter[3].input(HX_theta);

				if(filter_num == 5)
				{
					filter_num = 0;
					filter_flag = true;
					miX_nH = filter[0].output();
					miY_nH = filter[1].output();
					theta = filter[2].output();
					HX_theta = filter[3].output();					
					//	printf("midXnH,midYnH %lf,%lf, theta %lf, HX %lf\n", miX_nH, miY_nH, theta, HX_theta);
				}
			}
			...
			if(enter_flag && filter_flag)
			{
				data_index++;
				//if(data_index%2 == 0)
				{
					printf("X:%lf,Y:%lf,Angle:%lf\n",miY_nH,miX_nH,HX_theta);
					LOG_INFO(Recharge_logger,"X:"<<miY_nH<<" Y:"<<miX_nH<<" Angle:"<<HX_theta);
				}
				if(!alreadyadd)
				{
					All_time ++;
				...
				//}
				if( miY_nH <= DistYtoDockStation && miY_nH >= (DistYtoDockStation -30) )// && fabs(miX_nH) <=30 && fabs(miY_nH - prevOKmidY) <=77 ) //0.6m // x:wn'2cm
				{
					Sucess_time ++;
					isDocked=true;
					...
					SubPubManager::Instance()->m_controltaskresult.GetPublisher()->publish(feedbackInfo);
					//printf("Enter Station Successfully! Once-Successful Percent is %lf of %d times!\n",100-Sucess_time_nd*100.0/All_time,All_time);
					printf("Enter Station Successfully!\n");
					LOG_INFO(Recharge_logger,"Enter Station Successfully!");
				...
				{
					if( miY_nH < Conf_distmin)
					{
						//isNearDock = false;
						//yawRateK = (0-miX_nH)*YawRateScaleF*0.3;
						//speedK = 0;
						//cur_vel.set_vx(constVel*0.5/1000.0);
						//cur_vel.set_vy(0);
						//cur_vel.set_w( yawRateK );
						LOG_ERROR(Recharge_logger,"Last Dist! Almost Enter Station!");
						printf("Last Dist! Almost Enter Station!\n");
						enter_flag = false;
						last_flag = true;
						preLeave_odom = curLeave_odom;
					}
					...
					else if( miY_nH<Conf_distmax && miY_nH>Conf_distmin)
					{
						...
						if(fabs((miX_nH+pre_minX)/2.0)>recheck_dist) // 3cm
						{
							Sucess_time_nd ++;
							if(back_time_flag>2)
							{
								...
								cur_vel.set_vy(0);
								cur_vel.set_w(0);
								// printf(" wn'(90cm,93cm) theta %.2lf, miXdock_nH %.2lf, Deviation too LARGE! Enter Station Failed!\n", theta, miXdock_nH);
								// printf(" wn'(90cm,93cm) theta %.2lf, miXdock_nH %.2lf, Deviation too LARGE! Enter Station Failed!\n", theta, miXdock_nH);
								printf("Already Retry Twice! Recharge Failed!");
								LOG_ERROR(Recharge_logger,"Already Retry Twice! Recharge Failed!");
								feedbackInfo.set_flag(feedbackInfo.ECTF_ROLLER_ENTER_STATION);
								feedbackInfo.set_result(1); // enter roller failed
								SubPubManager::Instance()->m_controltaskresult.GetPublisher()->publish(feedbackInfo);
								enter_flag = false;
								Recharge_enable = false;
								back_flag = false;
							...
							{
								LOG_ERROR(Recharge_logger,"Will Try Again! Come On!");
								printf("Error Is Too Large!Will Try Again!\n");
								enter_flag = false;
								back_flag = true;
								preLeave_odom = curLeave_odom;
							}
						...
						else if( fabs(HXdelta) > 1.0 )
						{
							...
							speedK = 35;
							cur_vel.set_vx(constVel*0.1/1000.0);  //==5
							cur_vel.set_vy(0);
							cur_vel.set_w( yawRateK );
						}
						else {
							...
							cur_vel.set_vx(constVel*0.1/1000.0);
							cur_vel.set_vy(0);
					...
					{
						isNearDock = false;
						if( fabs(miX_nH)<10 ) //1cm
						{
							if( fabs(HX_theta-90)>1.0 ) { // ie. |HX_theta -90| >1.0
								// printf("\n 93cm+' vx =35; yawRate %lf _eg. |HXdelta|>1.0", HX_theta-90);
								yawRateK = theta*YawRateScaleF;
								speedK = 35;
								cur_vel.set_vx(constVel*miY_nH/1500000.0);
								cur_vel.set_vy(0);
						...
						else
						{
							if(fabs(miX_nH)>300)//see LiuJY'commented' 30cm
							{
								...
								else
								{
									...
									speedK = 35;
									if(miX_nH>0)
										yawRateK = (HX_theta-120)*YawRateScale + theta*YawRateScale;
									else
										yawRateK = (HX_theta-60)*YawRateScale + theta*YawRateScale;
							...
							else // (fabs(miX_nH)<=300)
							{
								...
								cur_vel.set_vx(constVel*miY_nH/1500000.0);
								cur_vel.set_vy(0);
								cur_vel.set_w( yawRateK*(fabs(miX_nH)/100.0+1) ); // (HX_theta-(90+miX_nH*0.1))*YawRateScale + theta*YawRateScale
				...
				SubPubManager::Instance()->m_robotspeed.GetPublisher()->publish(cur_vel);
				// printf("miXnH:%lf cm, HXdelta:%lf, theta:%lf\n", miX_nH*0.1, HXdelta, theta);
			...
			if(back_flag)
			{
				...
				cur_vel.set_vy(0);
				cur_vel.set_w(0);
				//cur_vel.set_w(miX_nH*YawRateScaleF*0.3);
				back_dist = curLeave_pose.x*curLeave_pose.x + curLeave_pose.y*curLeave_pose.y;
				SubPubManager::Instance()->m_robotspeed.GetPublisher()->publish(cur_vel);

				if(back_dist>0.5*0.5)
				{
					back_time_flag ++;
					printf("the %d try over!\n",back_time_flag);
					preLeave_odom = curLeave_odom;
					back_dist = 0;
					curLeave_pose.x = 0.0; curLeave_pose.y = 0.0; curLeave_pose.theta = 0.0;
					back_flag = false;
			...
			if(last_flag)
			{
				...
				cur_vel.set_vy(0);
				cur_vel.set_w(0);
				last_dist = curLeave_pose.x*curLeave_pose.x + curLeave_pose.y*curLeave_pose.y;
				//printf("Last_dist:%lf\n",last_dist);
				SubPubManager::Instance()->m_robotspeed.GetPublisher()->publish(cur_vel);

				if(last_dist>the_last_dist*the_last_dist)
				{
					preLeave_odom = curLeave_odom;
					last_dist = 0;
					curLeave_pose.x = 0.0; curLeave_pose.y = 0.0; curLeave_pose.theta = 0.0;
...
//	printf("get Navi_task it's %d\n",recharge_motion.flag());
	...
	if(enter_flag)
	{
		...
		LOG_INFO(Recharge_logger,"GET CHARGE_IN TASK!");
		pre_minX = 0;
		pre_theta = 0;
		no_find_times = 0;
		leave_flag = false;
	...
	{
		printf("Get Charge_Out Task\n");
		LOG_INFO(Recharge_logger,"GET CHARGE_OUT TASK!");
		no_find_times = 0;
		enter_flag = false;
		leave_flagTo = false;
		preLeave_odom = curLeave_odom;
...
int main(int argc,char**argv)
{
	...
	double RobotStationdist = 0.53;
	double RecheckDist = 0.033;
	double RechargeSpeed = 0.1;
	double Leave_Station = 0.7;
	double Last_Dist = 0.2;

	NRF_ParamReader::Instance()->readParams(PATH_PARAM.c_str()); // "./params/NR_AGV_param.xml"
	{
		DECLARE_PARAM_READER_BEGIN(Recharge)
		READ_PARAM(RobotStationdist)
		READ_PARAM(RecheckDist)
		READ_PARAM(RechargeSpeed)
		READ_PARAM(Leave_Station);
		READ_PARAM(Last_Dist);
		DECLARE_PARAM_READER_END
	}

	DistYtoDockStation		= RobotStationdist*1000;// 520 // 60cm'
	recheck_dist			= RecheckDist*1000;
	constVel				= RechargeSpeed*1000;
	leave_station_dist		= Leave_Station;
	the_last_dist			= Last_Dist;
	if(leave_station_dist>1.0)	leave_station_dist = 1.0;
	if(leave_station_dist<0.4)	leave_station_dist = 0.4;
	if(constVel>200)	constVel = 200;
	if(the_last_dist>0.27)	the_last_dist = 0.27;
	printf("Recharge Module Open! Read Parameter Completed!\n");
	LOG_INFO(Recharge_logger,"Recharge Module Open! Read Parameter Completed! STATION_DIST:"<<RobotStationdist<<" CHECK_DIST:"<<RecheckDist<<" SPEED:"<<RechargeSpeed<<" Leave_Station:"<<leave_station_dist);
	...
	SubPubManager::Instance()->m_controltaskresult.Initialize(Topic::Topic_CtrlTaskResult, NULL);		//发布充电反馈信息
